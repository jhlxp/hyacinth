...
#define DEFAULT_THRESHOLD 100000 // bytes
...
class NdpSrc : public PacketSink, public EventSource {
 ...
 public:
    ...
    inline bool is_flow_finished() const { return _flow_finished; }
    ...
    void setvlb(bool vlb) {_vlb = vlb;}
    bool isvlb() {return _vlb;}

    void replace_route(Route* newroute);

    virtual void rtx_timer_hook(simtime_picosec now,simtime_picosec period);
    void set_paths(vector<const Route*>* rt); // includes shortest path and vlb paths
    ...
    void set_num_shortest_paths(int num) { _num_shortest_paths = num;}
    int _num_shortest_paths;
    ...
    static uint64_t _threshold; // VLB threshold, bytes

    int _node_num;

    int _flow_src; // the sender (source) for this flow
    int _flow_dst; // the receiver (sink) for this flow
    int _coflow_group_id; // coflow/group id parsed from traffic/dag task
    bool _flow_finished;
    
    bool _vlb; // do we use VLB or not

public:
    // Pointer to the network topology instance.
    FlatTopology* dep_top = nullptr;

    // Pointer to the global retransmission timer scanner for NDP sources.
    NdpRtxTimerScanner* dep_ndpRtxScanner = nullptr;

    // Pointer to the global sink logger used for receiver sampling.
    NdpSinkLoggerSampling* dep_sinkLogger = nullptr;

    // Pull rate used for NDP connections (in relative line rate units).
    double dep_pull_rate = 1.0;

    // Initial congestion window size (in packets).
    int dep_cwnd = 30;

    // Flag to enable or disable VLB (Valiant Load Balancing) routing.
    int dep_VLB = 0;

    // Corresponding line index in the flow file (0-based).
    int rag_id = -1;

    // Pointer to the global DagManager instance for dependency tracking.
