...
/* */
uint64_t NdpSrc::_threshold = DEFAULT_THRESHOLD;
...
void NdpSrc::startflow(){
    ...
    // decide whether we use vlb routing or not
    // if src and dst are within the same rack, we don't do VLB
    //if (_flow_size <= _threshold || _paths.size() < 2) {
    //	_vlb = false;
    //}
    //else
    //	_vlb = true;
...
bool NdpSrc::is_bad_path() {
    ...
    // NDP can get confused in an flat when there's congestion...
...
*/
    ...
    p = NdpPacket::newpkt(_flow, *_route, nack.ackno(), 0, _pkt_size, true,
...
void NdpSrc::processAck(const NdpAck& ack) {
    ...
    */
	...
	// 		<< " diff=" << ((long long)_flow_size - (long long)cum_ackno)
	// 		<< " last_acked=" << _last_acked
	// 		<< " flight_size=" << _flight_size
	// 		<< " now=" << timeAsMs(eventlist().now())
	// 		<< " start=" << timeAsMs(get_start_time())
	// 		<< std::endl;

    if (cum_ackno >= _flow_size && !_flow_finished){
	   //cout << "Flow " << nodename() << " finished at " << timeAsMs(eventlist().now()) << endl;
        ...
        //cout << "flowID " << get_id() << " bytes " << get_flowsize() << " FCT_ms " << timeAsMs(eventlist().now() - get_start_time()) << endl;
        _flow_finished = true;
...
const Route* NdpSrc::choose_route() {
    ...
    // Note: permutation of paths not compatible with vlb implementation...
    ...
    case SCATTER_RANDOM:
		//ECMP + VLB if flow is larger than threshold
		assert(_paths.size() > 0);
        _crt_path = random()%_num_shortest_paths; // choose from amongst the shortest paths
        if (_vlb) {
            
            while (_crt_path < _num_shortest_paths) {

                // debug:
                //if (timeAsMs(eventlist().now()) > 552.370336) {
                //    cout << "_flow_src = " << _flow_src << ", _flow_dst = " << _flow_dst <<
                //        ", _crt_path = " << _crt_path << ", _num_shortest_paths = " << _num_shortest_paths << endl;
                //}

                _crt_path = random()%_paths.size(); // randomly pick a path from the set of VLB paths

            }
        }

        //cout << "_flow_src = " << _flow_src << ", _flow_dst = " << _flow_dst <<
        //    ", _crt_path = " << _crt_path << ", _num_shortest_paths = " << _num_shortest_paths << ", num_total_paths = " << _paths.size() << endl;
...
void NdpSrc::send_packet(NdpPull::seq_t pacer_no) {
	...
	//implement in a real system, so this is a rough proxy.
		...
		case PULL_BASED:
		{
	    	assert(_paths.size() > 0);
	    	const Route *rt = choose_route();
	    	p = NdpPacket::newpkt(_flow, *rt, _highest_sent+1, pacer_no, _pkt_size, false,
					  _paths.size()>0?_paths.size():1, last_packet); // modified
...
NdpSrc::retransmit_packet() {
	...
	case PULL_BASED:
	{
	    ...
	    p = NdpPacket::newpkt(_flow, *rt, seqno, 0, _pkt_size, true,
				  _paths.size()>0?_paths.size():1, last_packet);
	...
	case SINGLE_PATH:
	    p = NdpPacket::newpkt(_flow, *_route, seqno, 0, _pkt_size, true,
...
void NdpSrc::doNextEvent() {
    ...
    // std::cout << "[NDP-SRC] doNextEvent: flow_uid=" << rag_id
    //           << " at=" << timeAsUs(eventlist().now())/1e6 << "s"
    //           << " rtx_pending=" << _rtx_timeout_pending
    //           << std::endl;
...
void NdpSink::receivePacket(Packet& pkt) {
	...
	*/
    ...
    if (_last_packet_seqno > 0 && _cumulative_ack == _last_packet_seqno) {
        ...
        // cout << "[DEBUG-SINK-FIN]"
        //           << " uid=" << _src->rag_id
        //           << " last_seq=" << _last_packet_seqno
        //           << " cum_ack=" << _cumulative_ack
        //           << endl;
...
void NdpSink::send_ack(simtime_picosec ts, NdpPacket::seq_t ackno, NdpPacket::seq_t pacer_no) {
	...
	if (_route_strategy == SCATTER_RANDOM) {
	    ...
	    _crt_path = random()%_num_shortest_paths; // choose from amongst the shortest paths
        // always send ACKs / NACKs back on the short paths...
        //if (_src->isvlb()) {
        //    while (_crt_path < _num_shortest_paths)
        //        _crt_path = random()%_paths.size(); // randomly pick a path from the set of VLB paths
        //}
