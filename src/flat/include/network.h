...
typedef enum {IP, TCP, TCPACK, TCPNACK, NDP, NDPACK, NDPNACK, NDPPULL, NDPLITE, 
  NDPLITEACK, NDPLITEPULL, NDPLITERTS, ETH_PAUSE, XPDATA, XPCREDIT,SAMPLE} packet_type;
...
class Packet {
 ...
 public:
    ...
    // virtual void strip_payload() { assert(!_is_header); cout << "STRIP\n"; _is_header = true;};
    ...
    inline uint64_t flowid() const {return _flowid;}
    inline void set_flow_id(uint64_t id) {_flowid = id;}
    inline void set_fabricts(simtime_picosec ts) { _fabricts = ts;}
    inline simtime_picosec get_fabricts() { return _fabricts; }
    string str() const;
    ...
    void set_int(map<string, pktINT> ints) {_pkt_ints = ints;}
    void inc_nexthop() {_nexthop ++;}
 protected:
