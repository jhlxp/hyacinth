...
Queue::receivePacket(Packet& pkt) 
{
    ...
    _queuesize += pkt.size();
    updatePktIn(pkt.flowid());
    if (_queuesize > _max_reported_size) {
	_max_reported_size = _queuesize;
    }
...
mem_b
Queue::reportMaxqueuesize() {
    mem_b tmp = _max_reported_size;
    _max_reported_size = 0;
    return tmp;
}
...
PriorityQueue::completeService()
{
    ...
    // debug:
    //cout << "A packet was sent from a NIC at " << timeAsUs(eventlist().now()) << " us." << endl;
