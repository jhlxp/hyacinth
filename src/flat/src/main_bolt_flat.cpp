...
int main(int argc, char **argv) {
    ...
    if (flowfile.empty() || topfile.empty()) {
        cerr << "[bolt_flat] ERROR: -flowfile and -topfile are required." << endl;
        return 1;
    }
    if (outputfile.empty()) {
        cerr << "[bolt_flat] ERROR: -outputfile is required." << endl;
        return 1;
    }
    ...
    if (!input.is_open()) {
        cerr << "[bolt_flat] ERROR: cannot open flowfile: " << flowfile << endl;
        return 1;
    }
    ...
    while (getline(input, line)) {
        ...
        if (tokens.size() < 4) {
            cerr << "[bolt_flat] Skip malformed flow line " << line_no
        ...
        if (!parse_int64_token(tokens[0], flow_src64) ||
            ...
            cerr << "[bolt_flat] Skip malformed numeric fields at line " << line_no << endl;
            continue;
        }
        ...
        for (size_t tok_idx = 4; tok_idx < tokens.size(); ++tok_idx) {
            ...
            if (extra.rfind("path=", 0) == 0) {
                ...
                if (!parse_tor_path(extra.substr(5), explicit_tor_path, err)) {
                    cerr << "[bolt_flat] Invalid path token at line " << line_no
                         << ": " << err << endl;
        ...
        if (!has_explicit_path) {
            cerr << "[bolt_flat] Skip flow line " << line_no
