    ...
    */
    ...
    if (cum_ackno >= _flow_size && !_flow_finished){
	   //cout << "Flow " << nodename() << " finished at " << timeAsMs(eventlist().now()) << endl;
        ...
        // Keep the same flow-completion log format as TCP-family:
        // "<group_id> FCT <src> <dst> <bytes> <fct_ms> <start_ms>"
        // cout << get_coflow_group_id() << " FCT " << get_flow_src() << " " << get_flow_dst() << " " << get_flowsize() <<
        ...
        OUTPUT_LOG << get_coflow_group_id() << " "
            ...
            " " << timeAsMs(eventlist().now() - get_start_time()) << " " << timeAsMs(get_start_time()) << "\n";
        return;
    }
