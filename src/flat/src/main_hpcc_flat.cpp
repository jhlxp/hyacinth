...
void exit_error(char* progr) {
    cerr << "Usage " << progr
         ...
         << "[-simtime s] [-utiltime s] [-q pkts] [-eta x] [-maxstage n]"
         << endl;
    exit(1);
}
...
int main(int argc, char **argv) {
    ...
    while (i < argc) {
        if (!strcmp(argv[i], "-o")) {
            ...
            nn = atof(argv[i + 1]);
            i++;
        } else if (!strcmp(argv[i], "-maxstage")) {
    ...
    if (flowfile.empty() || topfile.empty()) {
        cerr << "[hpcc_flat] ERROR: -flowfile and -topfile are required." << endl;
        return 1;
    }
    if (outputfile.empty()) {
        cerr << "[hpcc_flat] ERROR: -outputfile is required." << endl;
        return 1;
    }
    ...
    FlatTopology* top = new FlatTopology(queuesize, &logfile, &eventlist, HPCC, topfile);

    ifstream input(flowfile);
    if (!input.is_open()) {
        cerr << "[hpcc_flat] ERROR: cannot open flowfile: " << flowfile << endl;
        return 1;
    }
    ...
    while (getline(input, line)) {
        ...
        if (tokens.size() < 4) {
            cerr << "[hpcc_flat] Skip malformed flow line " << line_no
        ...
        if (!parse_int64_token(tokens[0], flow_src64) ||
            ...
            cerr << "[hpcc_flat] Skip malformed numeric fields at line " << line_no << endl;
            continue;
        }
        ...
        for (size_t tok_idx = 4; tok_idx < tokens.size(); ++tok_idx) {
            ...
            if (extra.rfind("path=", 0) == 0) {
                ...
                if (!parse_tor_path(extra.substr(5), explicit_tor_path, err)) {
                    cerr << "[hpcc_flat] Invalid path token at line " << line_no
                         << ": " << err << endl;
        ...
        if (!has_explicit_path) {
            cerr << "[hpcc_flat] Skip flow line " << line_no
        ...
        TcpSrc* flowSrc = new HPCCSrc(NULL, NULL, eventlist, flow_src, flow_dst, max_stage, nn);
