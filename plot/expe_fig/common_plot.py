...
def save_png_and_pdf(fig: plt.Figure, out_png: str, logger: FileLogger) -> None:
    ...
    fig.savefig(out_png, dpi=OUTPUT_DPI, bbox_inches="tight")
    fig.savefig(out_pdf, dpi=OUTPUT_DPI, bbox_inches="tight")
...
def detect_latest_scenario_root(repo_root: str, exp_name: str) -> str:
    parent = os.path.join(repo_root, "experiments", "expe_logs_10round", exp_name)
...
def plot_grouped_bar(
    ...
    if is_relative:
        ax.axhline(1.0, linestyle="--", linewidth=1.1, color="#444444", alpha=0.8)
...
def plot_single_case_bar(
    ...
    labels = [ALGORITHM_LABELS.get(s, s) for s in shown_sched]
