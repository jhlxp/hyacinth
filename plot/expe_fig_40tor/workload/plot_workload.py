...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot section figures for workload experiments.")
    parser.add_argument("--scenario_root", default="", help="Scenario batch root (default: latest workload batch).")
    parser.add_argument("--out_dir", default="", help="Output dir (default: expe_fig/workload/figures).")
    parser.add_argument("--min_success_seeds", type=int, default=10)
    args = parser.parse_args()

    repo_root = os.path.abspath(os.path.join(EXPE_FIG_DIR, "..", ".."))
    scenario_root = args.scenario_root or detect_latest_scenario_root(repo_root, "workload")
    out_dir = args.out_dir or os.path.join(SCRIPT_DIR, "figures")
    run_section(
        exp_name="workload",
        section_title="Workload-CDF Sweep",
