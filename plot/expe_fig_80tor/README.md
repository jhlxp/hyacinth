...
- `fixed_dynamic/plot_fixed_dynamic.py`
  - Variant-only figure set:
    - `ocs_eps_preset_greedy` as `Preset 0.2`
    - `ocs_eps_preset_dynamic_greedy` as `Dynamic`
  - Draws merged all-scenario line plots only (dedup duplicate base-case).
  - Outputs:
    - `fixed_dynamic/figures/*.png`
    - logs in corresponding `.log` files.
- `run_all_expe_fig.sh`
