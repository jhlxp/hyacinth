...
TARGET_LABELS = {
    ...
    "ocs_eps_preset_greedy": "Hyacinth-static-20%",
}
TARGET_COLORS = {
...
MERGED_X_TICK_FONT_SIZE = TICK_FONT_SIZE - 2
...
def run_merged_line_plots(repo_root: str, out_root: str, min_success_seeds: int) -> None:
    ...
    try:
        ...
        for exp_name in SCENARIOS:
            ...
            for case_base in cases:
                ...
                points_map[key] = {
                    ...
                    "case": case_base,
                    "label": case_base,
                    "coflow": {s: coflow[0][i] for i, s in enumerate(TARGET_SCHEDULERS)},
                    "solve": {s: solve[0][i] for i, s in enumerate(TARGET_SCHEDULERS)},
                }
        ...
        coflow_series = {
            s: [float(p["coflow"][s]) for p in points] for s in TARGET_SCHEDULERS
        }
        solve_series = {
            s: [float(p["solve"][s]) for p in points] for s in TARGET_SCHEDULERS
        }
