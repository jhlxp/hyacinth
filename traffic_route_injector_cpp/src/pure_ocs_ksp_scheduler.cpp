...
PureOcsKspScheduler::PureOcsKspScheduler(int kspK) : kspK_(kspK) {
    if (kspK_ <= 0) {
        throw std::runtime_error("pure_ocs_ksp requires kspK > 0.");
    }
}

uint64_t PureOcsKspScheduler::pairKey(int s, int t) {
    return (static_cast<uint64_t>(static_cast<uint32_t>(s)) << 32) |
           static_cast<uint32_t>(t);
}

void PureOcsKspScheduler::prepare(const SchedulerContext& ctx) const {
...
bool PureOcsKspScheduler::countsSolveTime() const {
    // KSP routes are treated as precomputed/offline.
    return false;
}

std::string PureOcsKspScheduler::name() const {
    return "pure_ocs_ksp";
}

std::vector<ScheduledFlow> PureOcsKspScheduler::scheduleJob(const Job& job,
    ...
    for (const auto& flow : job.flows) {
        ...
        PlannedFlow pf;
        pf.flow = flow;
        const size_t pick = pickCandidateIndex(flow, candidates.size());
    ...
    while (done < static_cast<int>(planned.size())) {
        ...
        if (active.empty()) {
            ...
            if (!std::isfinite(nextRelease)) {
                throw std::runtime_error("No active KSP flow and no future release; invalid state.");
            }
            now = nextRelease;
            continue;
        }
        ...
        for (int idx : active) {
            ...
            for (const auto& edge : pf.edges) {
                ...
                if (it == edgeActiveCount.end() || it->second <= 0) {
                    throw std::runtime_error("KSP share accounting failed for an active edge.");
                }
                rate = std::min(rate, cap / static_cast<double>(it->second));
            }
            if (!(rate > 0.0)) {
                throw std::runtime_error("KSP shared rate must be positive.");
            }
        ...
        if (!std::isfinite(dt)) {
            throw std::runtime_error("KSP event simulation reached non-finite dt.");
        }
        if (dt <= kEps) {
            ...
            // Degenerate numeric corner: force-complete the smallest remaining active flow.
