...
uint64_t EpsEcmpScheduler::pairKey(int s, int t) {
    return (static_cast<uint64_t>(static_cast<uint32_t>(s)) << 32) |
           static_cast<uint32_t>(t);
}

void EpsEcmpScheduler::prepare(const SchedulerContext& ctx) const {
    if (ctx.numEps <= 0) {
        throw std::runtime_error("eps_ecmp requires num_eps > 0.");
    }
    ...
    cachedNumEps_ = ctx.numEps;
    ecmpCache_.clear();
    ecmpCache_.reserve(static_cast<size_t>(ctx.numTor) * static_cast<size_t>(ctx.numTor));
    ...
    for (int s = 0; s < ctx.numTor; ++s) {
        for (int t = 0; t < ctx.numTor; ++t) {
            ...
            auto candidates = enumerateSingleHopEpsPaths(s, t, ctx.capacity, ctx.numTor, ctx.numEps);
            ecmpCache_[pairKey(s, t)] = std::move(candidates);
        }
    }
}

bool EpsEcmpScheduler::countsSolveTime() const {
    // ECMP path set is treated as precomputed/offline.
    return false;
}

std::string EpsEcmpScheduler::name() const {
    return "eps_ecmp";
}

std::vector<ScheduledFlow> EpsEcmpScheduler::scheduleJob(const Job& job,
    ...
    for (const auto& flow : job.flows) {
        ...
        const auto it = ecmpCache_.find(pairKey(flow.torSrc, flow.torDst));
        if (it == ecmpCache_.end() || it->second.empty()) {
            throw std::runtime_error("No EPS-ECMP candidate path found for flowId=" +
    ...
    while (done < static_cast<int>(planned.size())) {
        ...
        if (active.empty()) {
            ...
            if (!std::isfinite(nextRelease)) {
                throw std::runtime_error("No active EPS-ECMP flow and no future release; invalid state.");
            }
            now = nextRelease;
            continue;
        }
        ...
        for (int idx : active) {
            ...
            for (const auto& edge : pf.edges) {
                ...
                if (it == edgeActiveCount.end() || it->second <= 0) {
                    throw std::runtime_error("EPS-ECMP share accounting failed for an active edge.");
                }
                rate = std::min(rate, cap / static_cast<double>(it->second));
            }
            if (!(rate > 0.0)) {
                throw std::runtime_error("EPS-ECMP shared rate must be positive.");
            }
        ...
        if (!std::isfinite(dt)) {
            throw std::runtime_error("EPS-ECMP event simulation reached non-finite dt.");
        }
        if (dt <= kEps) {
