...
# Standalone full pipeline for infer-groups traffic (N=1, interval=50ms):
#   0) generate infer .htsim (overwrite)
...
TRAFFIC_IN="${TRAFFIC_IN:-$HTSIM_ROOT/experiments/traffic_coflow/infer_80racks_8c_mix4L3M3S_frag0p5_ng1_int50ms_1.000s.htsim}"
...
TRAFFIC_MODE="${TRAFFIC_MODE:-infer_groups}"
TRAFFIC_INFER_GROUPS="${TRAFFIC_INFER_GROUPS:-1}"
TRAFFIC_INFER_INTERVAL_MS="${TRAFFIC_INFER_INTERVAL_MS:-50}"
TRAFFIC_INFER_MODEL_PP1_SPREAD_MS="${TRAFFIC_INFER_MODEL_PP1_SPREAD_MS:-100}"
TRAFFIC_INFER_PP_JITTER_MS="${TRAFFIC_INFER_PP_JITTER_MS:-5}"
...
RUN_TAG="${RUN_TAG:-run_$(date +%Y%m%d_%H%M%S)_infer_80racks_8c_mix4L3M3S_frag0p5_ng1_int50ms}"
