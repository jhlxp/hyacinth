...
BATCH_DIR="$HTSIM_ROOT/experiments/expe_logs_10round/${EXPERIMENT_TYPE}/$BATCH_TAG"
mkdir -p "$BATCH_DIR"
...
for ((i=0; i<${#CASE_TAGS[@]}; ++i)); do
  ...
  echo "$case_tag,$traffic_seed,$model_mix,$frag,$load,$nrack,$workload,$status,$run_dir" >> "$BATCH_SUMMARY"
done
