...
EXPERIMENT_TYPE="start_spread"
BATCH_TAG="${BATCH_TAG:-batch_$(date +%Y%m%d_%H%M%S)_${EXPERIMENT_TYPE}}"
BATCH_DIR="$HTSIM_ROOT/experiments/expe_logs/${EXPERIMENT_TYPE}/$BATCH_TAG"
mkdir -p "$BATCH_DIR"
...
run_case() {
  ...
  local model_pp1_spread_ms="${CASE_PP1_SPREAD_MS[$idx]}"

  local run_stamp
  run_stamp="$(date +%Y%m%d_%H%M%S)"
  local run_dir="$BATCH_DIR/run_${idx}_${run_stamp}_${case_tag}"
  ...
  python3 "$TRAFFIC_GEN_PY" \
    ...
    --infer-model-pp1-spread-ms "$model_pp1_spread_ms" \
...

  {
    ...
    echo "traffic_infer_model_pp1_spread_ms=$model_pp1_spread_ms"
  ...
  if [[ ${#ok_scheds[@]} -gt 0 ]]; then
    ...
    if [[ "$has_dynamic" == "1" ]]; then
      ...
      MPLCONFIGDIR="$mplconfigdir" python3 "$PLOT_COFLOW_AVG_REL" \
        ...
        --log_glob "*.log" \
        --schedulers "$(join_by_comma "${ok_scheds[@]}")" \
        --title "Avg Relative Coflow CCT ($EXPERIMENT_TYPE:$case_tag, dynamic=1)" \
        --out_csv "$summary_dir/coflow_avg_relative_summary.csv" \
        --out_png "$summary_dir/coflow_avg_relative_summary.png" \
        > "$summary_dir/plot_coflow_avg_rel.log" 2>&1 || echo "[warn] avg-relative-coflow plot failed: $case_tag"
    else
...
if [[ ${#CASE_TAGS[@]} -ne ${#CASE_MODEL_MIX[@]} || \
      ...
      ${#CASE_TAGS[@]} -ne ${#CASE_PP1_SPREAD_MS[@]} ]]; then
  echo "[fatal] case array length mismatch" >&2
  exit 1
fi

BATCH_SUMMARY="$BATCH_DIR/batch_summary.csv"
echo "case_tag,model_mix,frag,load,nrack,workload,pp1_spread_ms,status,run_dir" > "$BATCH_SUMMARY"
...
for ((i=0; i<${#CASE_TAGS[@]}; ++i)); do
  ...
  model_pp1_spread_ms="${CASE_PP1_SPREAD_MS[$i]}"
  ...
  echo "$case_tag,$model_mix,$frag,$load,$nrack,$workload,$model_pp1_spread_ms,$status,$LAST_RUN_DIR" >> "$BATCH_SUMMARY"
done
