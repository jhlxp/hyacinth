...
if [[ ${#CASE_TAGS[@]} -ne ${#CASE_MODEL_MIX[@]} || \
      ...
      ${#CASE_TAGS[@]} -ne ${#CASE_WORKLOADS[@]} ]]; then
  echo "[fatal] case array length mismatch" >&2
  exit 1
fi

BATCH_SUMMARY="$BATCH_DIR/batch_summary.csv"
echo "case_tag,model_mix,frag,load,nrack,workload,status,run_dir" > "$BATCH_SUMMARY"
...
for ((i=0; i<${#CASE_TAGS[@]}; ++i)); do
  ...
  echo "$case_tag,$model_mix,$frag,$load,$nrack,$workload,$status,$LAST_RUN_DIR" >> "$BATCH_SUMMARY"
done
