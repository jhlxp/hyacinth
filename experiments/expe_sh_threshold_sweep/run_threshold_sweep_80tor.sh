...
# =============================================================================
# Threshold Sweep Experiment (80tor base, 10-round multi-seed)
# =============================================================================
...
#
# This 80tor variant keeps outputs under expe_logs_threshold_sweep/threshold_sweep
# and uses a timestamped batch tag distinct from the 40tor script.
#
...
#
# Base case: 80-rack, mix=4L,3M,3S, frag=0.5, workload=fbcoco.csv
# =============================================================================
...
BATCH_TAG="${BATCH_TAG:-batch_$(date +%Y%m%d_%H%M%S)_${EXPERIMENT_TYPE}_80tor}"
...
# Fixed baseline knobs (same as 80tor mix base case)
HOSTS_PER_RACK=8
GPUS_PER_HOST=8
NRACK=80
MODEL_MIX="4L,3M,3S"
...
run_one_config() {
  ...
  local traffic_case_tag="mix4L3M3S_seed${seed}"
...
for ((i=0; i<${#FULL_SCHEDS[@]}; ++i)); do
  ...
  echo "$scheduler,$thresh_tag,$thresh_value,$seed,$num_eps,$TOPO_EPS1,$run_dir/native_traffic/infer_mix4L3M3S_seed${seed}_${NRACK}racks.htsim,$status,$run_dir" >> "$BATCH_SUMMARY"
...
if [[ ${#OK_THRESH_TAGS[@]} -ge 2 ]]; then
  ...
  for seed in $(seq "$TRAFFIC_SEED_START" "$TRAFFIC_SEED_END"); do
    ...
    MPLCONFIGDIR="$BATCH_DIR/.mplconfig" python3 "$PLOT_FCT_BYTES" \
      ...
      --title "Flow Size vs Completion Time (80tor Threshold Sweep seed=$seed)" \
    ...
    if [[ "$has_dynamic" == "1" ]]; then
      MPLCONFIGDIR="$BATCH_DIR/.mplconfig" python3 "$PLOT_FCT_AVG_REL" \
        ...
        --schedulers "$SEED_OK_CSV" \
        --title "Avg Relative FCT vs Dynamic (80tor Threshold Sweep seed=$seed)" \
      ...
      MPLCONFIGDIR="$BATCH_DIR/.mplconfig" python3 "$PLOT_COFLOW_AVG_REL" \
        ...
        --title_template "{p} Relative Coflow CCT vs Dynamic (80tor Threshold Sweep seed=$seed)" \
...
echo ""
echo "==============================================="
echo "  80tor Threshold Sweep Experiment Complete"
echo "==============================================="
