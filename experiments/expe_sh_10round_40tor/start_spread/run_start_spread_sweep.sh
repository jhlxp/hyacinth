...
BATCH_DIR="$HTSIM_ROOT/experiments/expe_logs_10round_40tor/${EXPERIMENT_TYPE}/$BATCH_TAG"
mkdir -p "$BATCH_DIR"
...
BASE_CASE_PP1_SPREAD_MS=("${CASE_PP1_SPREAD_MS[@]}")
...
for ((bi=0; bi<${#BASE_CASE_TAGS[@]}; ++bi)); do
  for seed in $(seq "$TRAFFIC_SEED_START" "$TRAFFIC_SEED_END"); do
    ...
    CASE_PP1_SPREAD_MS+=("${BASE_CASE_PP1_SPREAD_MS[$bi]}")
    CASE_SEEDS+=("$seed")
  done
done
...
if [[ ${#CASE_TAGS[@]} -ne ${#CASE_MODEL_MIX[@]} || \
      ...
      ${#CASE_TAGS[@]} -ne ${#CASE_PP1_SPREAD_MS[@]} || \
...
echo "case_tag,seed,model_mix,frag,load,nrack,workload,pp1_spread_ms,status,run_dir" > "$BATCH_SUMMARY"
...
for ((i=0; i<${#CASE_TAGS[@]}; ++i)); do
  ...
  printf '%s,%s,"%s",%s,%s,%s,"%s",%s,%s,"%s"\n' "$case_tag" "$traffic_seed" "$model_mix" "$frag" "$load" "$nrack" "$workload" "$model_pp1_spread_ms" "$status" "$run_dir" >> "$BATCH_SUMMARY"
done
